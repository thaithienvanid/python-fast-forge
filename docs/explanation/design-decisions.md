...
13. [Use Atlas for Schema Migrations](#adr-013-use-atlas-for-schema-migrations)
...
## ADR-013: Use Atlas for Schema Migrations

**Status**: Accepted

**Context**:
- Need modern, declarative schema management
- Want automatic drift detection between code and database
- Require support for advanced PostgreSQL features (functions, triggers, views)
- Need safety checks to prevent destructive changes
- CI/CD integration must be seamless
- Schema visualization would improve developer experience
- Prefer SQL migrations over imperative Python for clarity

**Decision**:
Use Atlas for database schema management, with SQLAlchemy models as the source of truth.

**Consequences**:

*Positive*:
- **Declarative approach**: SQLAlchemy models define schema, migrations auto-generated
- **Advanced PostgreSQL support**: Handles functions, triggers, views, procedures, extensions
- **Drift detection**: Automatically catches when database state diverges from code
- **Safety built-in**: Lint rules prevent destructive changes, catch data-dependent operations
- **Excellent CI/CD**: Native GitHub Actions support, automatic PR schema checks
- **Pure SQL migrations**: Easy to review, version control, and understand
- **Schema visualization**: Interactive browser-based schema inspection and ERD
- **Developer experience**: Simple workflow with `make migrate` and `make migrate-create`
- **Timestamped migrations**: Clear ordering with `YYYYMMDDHHMMSS_description.sql` format

*Negative*:
- Newer tool with smaller community than traditional migration tools
- Requires Docker for local database validation (or local PostgreSQL instance)
- Team needs to learn Atlas workflow and concepts
- Less third-party integrations compared to mature alternatives

**Implementation**:
- Configured environments in `atlas.hcl` (local, dev, staging, prod)
- Created Makefile targets for common workflows
- Set up automatic drift detection in CI/CD
- Added schema linting to catch issues before deployment

**Alternatives Considered**:
- **Alembic**: Python-based migrations, mature ecosystem, but lacks drift detection and advanced PostgreSQL features
- **Manual SQL scripts**: Full control but error-prone, no safety checks, no versioning
- **Flyway**: JVM-based, production-ready, but no SQLAlchemy integration, less Pythonic
- **Liquibase**: XML/YAML-based, complex, overkill for our needs, steep learning curve

**References**:
- [Atlas Migration Explanation](atlas-migration.md)
- [Migration How-To Guide](../how-to/database-migrations.md)
- [Command Reference](../reference/atlas-commands.md)
