...
# Alembic migration
def upgrade():
    op.add_column('users', sa.Column('tenant_id', postgresql.UUID(), nullable=True))

    # Create default tenant
    op.execute("INSERT INTO tenants (id, name) VALUES (gen_random_uuid(), 'Default')")

    # Set all existing rows to default tenant
    op.execute("UPDATE users SET tenant_id = (SELECT id FROM tenants WHERE name = 'Default')")
