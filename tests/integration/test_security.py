...
class TestSecurityVulnerabilities:
    ...
    def test_xss_attempt_in_user_fields(self, client: TestClient) -> None:
        """Test that XSS attempts don't cause server errors.

        Verifies that HTML/JavaScript in user input is handled gracefully.
        XSS protection should be implemented at render time (output encoding),
        not at storage time. The API should accept the data without crashing.
        """
        xss_payloads = [
        ...
        for payload in xss_payloads:
            ...
            assert response.status_code in [201, 400, 422], (
                f"XSS payload should be handled gracefully, got {response.status_code}"
            )
            # If accepted, verify no server error occurred
            if response.status_code == 201:
                data = response.json()
                assert "full_name" in data  # Response is valid
    ...
    def test_special_characters_handling(self, client: TestClient) -> None:
        ...
        for payload in special_char_payloads:
            ...
            if response.status_code != 201:
                ...
                # Check for either FastAPI's default format or our custom error format
