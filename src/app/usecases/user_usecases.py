...
from src.app.decorators import handle_integrity_errors
...
class ListUsersUseCase:
    ...
    async def execute(
        ...
        Design Pattern:
            Returns both data and total count for proper pagination
        """
        if skip < 0:
        ...
        # Get paginated users
        users = await self._repository.get_all(skip=skip, limit=limit, tenant_id=tenant_id)

        # Get total count for pagination metadata
        total = await self._repository.count_all(tenant_id=tenant_id)
...
class CreateUserUseCase:
    ...
    This use case demonstrates clean architecture and event-driven design:
    - Business logic is focused and testable
    - Infrastructure concerns handled via events
    ...
    @handle_integrity_errors  # ✅ Decorator handles IntegrityError → ValidationError
    async def execute(
        self,
        email: str,
        username: str,
        ...
        Design Pattern:
            - Decorator Pattern: @handle_integrity_errors eliminates boilerplate
            - Event-Driven: Domain events decouple business logic from infrastructure

        SOLID Principles:
            - Single Responsibility: Only creates user, events handle side effects
            - Open/Closed: Can add new event handlers without modifying this code
            - Dependency Inversion: Depends on event abstractions, not Temporal
        """
...
class UpdateUserUseCase:
    ...
    async def execute(
        ...
        Design Pattern:
            - Decorator Pattern: @handle_integrity_errors eliminates boilerplate
            - Event-Driven: Publishes UserUpdatedEvent for audit trail
        """
        user = await self._repository.get_by_id(user_id)
        if not user:
            raise EntityNotFoundError(f"User with ID {user_id} not found")
        ...
        # Update user (decorator handles IntegrityError)
...
class BatchCreateUsersUseCase:
    ...
    async def execute(
        ...
        Design Pattern:
            - Decorator Pattern: @handle_integrity_errors eliminates boilerplate
            - Unit of Work: Ensures transactional consistency for batch operations
        """
        if not users_data:
        ...
        # Decorator handles any IntegrityError that may occur
        async with self._uow_factory() as uow:
            ...
            if existing_users_by_username:
                ...
                raise ValidationError(f"Users with usernames {existing_usernames} already exist")
