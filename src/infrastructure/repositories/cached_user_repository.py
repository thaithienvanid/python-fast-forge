...
class CachedUserRepository(CachedBaseRepository[User], IUserRepository[User]):
    ...
    async def count_all(self, tenant_id: UUID | None = None) -> int:
        """Count total users without filters (pass-through, no caching).

        Count operations are not cached as they can change frequently and
        caching them could lead to stale counts.
        ...
        """
        return await self._user_repository.count_all(tenant_id)
